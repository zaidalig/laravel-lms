<div class="card-footer bg-white d-flex flex-wrap align-items-center justify-content-between gap-3 table-toolbar pagination-wrap">
    ...
    <div class="d-flex flex-wrap align-items-center gap-2"><form method="GET" class="d-flex align-items-center gap-2">
        ...
        <label class="small text-muted">Sort</label><select name="sort" class="form-select form-select-sm form-select-compact" onchange="this.form.submit()">@foreach(($sorts ?? ['created_at'=>'Created']) as $value=>$label)<option value="{{ $value }}" @selected(request('sort','created_at')===$value)>{{ $label }}</option>@endforeach</select>
